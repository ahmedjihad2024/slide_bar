...
class SideBarItem extends StatelessWidget {
  const SideBarItem(
      ...
      required this.expansionTileBuilder});
  ...
  final Widget Function(AdminMenuItem item, bool isSelected, Function() onTap)
      itemBuilder;

  final ExpansionTile Function(AdminMenuItem item, List<SideBarItem> children) expansionTileBuilder;
  ...
  Widget _buildTiles(BuildContext context, AdminMenuItem item) {
    ...
    if (item.children.isEmpty) {
      return itemBuilder(item, selected, () {
        if (onSelected != null) {
          onSelected!(item);
        }
      });
      ...
      //   leading: _buildIcon(item.widget, selected),
    ...
    final childrenTiles = item.children.map((child) {
      return SideBarItem(
        ...
        expansionTileBuilder: expansionTileBuilder,
      );
    }).toList();

    return Theme(
      data: Theme.of(context).copyWith(dividerColor: Colors.transparent),
      child: expansionTileBuilder(item, childrenTiles),
    );
  }
  ...
  Widget _buildIcon(Widget? widget, [bool selected = false]) {
    return widget != null ? widget : SizedBox();
  }
